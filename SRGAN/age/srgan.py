...
class AgeExperiment(Experiment):
    ...
    def dataset_setup(self):
        ...
        if model_architecture == 'vgg':
            dataset_path = '../SRGAN/imdb_wiki_data/imdb_preprocessed_224'
        else:
            dataset_path = '../SRGAN/imdb_wiki_data/imdb_preprocessed_128'
    ...
    def evaluation_epoch(self, settings, network, dataset, summary_writer, summary_name, comparison_value=None):
        ...
        fig, ax = plt.subplots()
        ax.plot(range(len(predicted_ages[:100])),predicted_ages[:100].reshape(-1), label="Predicted")
        ax.plot(range(len(ages[:100])),ages[:100].reshape(-1), label="True")
        ax.legend(loc='upper right')
        plt.title("Age Dataset FuzzyGAN Classification Injection: True RUL vs. Predicted RUL")
        plt.ylabel("Age")
        plt.xlabel("Index")
        plt.savefig("FuzzyGANAge_Classification_new.jpg")
