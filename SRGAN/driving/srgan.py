...
class DrivingExperiment(Experiment):
    ...
    def evaluation_epoch(self, settings, network, dataset, summary_writer, summary_name, comparison_value=None):
        ...
        fig, ax = plt.subplots()
        ax.plot(range(len(predicted_angles[:100])),predicted_angles[:100].reshape(-1), label="Predicted")
        ax.plot(range(len(angles[:100])),angles[:100].reshape(-1), label="True")
        ax.legend(loc='upper right')
        plt.title("Driving Dataset FuzzyGAN: True RUL vs. Predicted RUL")
        plt.ylabel("Angle")
        plt.xlabel("Index")
        plt.savefig("FuzzyGANDriving_Classification_new.jpg")
