...
class Discriminator(nn.Module):
    ...
    def forward(self, x):
        """The forward pass of the network."""
        out = leaky_relu(nn.utils.spectral_norm(self.layer1(x)), 0.05)    # (?, 64, 32, 32)
        out = leaky_relu(nn.utils.spectral_norm(self.layer2(out)), 0.05)  # (?, 128, 16, 16)
        out = leaky_relu(nn.utils.spectral_norm(self.layer3(out)), 0.05)  # (?, 256, 8, 8)
        out = leaky_relu(nn.utils.spectral_norm(self.layer4(out)), 0.05)  # (?, 512, 4, 4)
        self.features = out.view(out.size(0), -1)
        out = nn.utils.spectral_norm(self.layer5(out))
